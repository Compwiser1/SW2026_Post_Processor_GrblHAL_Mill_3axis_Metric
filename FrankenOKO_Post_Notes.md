# FrankenOKO GrblHal Post Processor — Session Continuity Notes

**Note on this file's history**: this file was originally a "pickup point"
covering one session's changes, with a note pointing to a separate, older
`FrankenOKO_Post_Notes.md` for deeper history (coolant, N-numbers,
operation notes de-dup, coordinate formatting, etc.). That older file
wasn't available when this repo was assembled — if you still have it,
send it over and I'll merge it in above this point rather than losing that
history. From this point forward, this is the single canonical
continuity file, carried in every release zip, appended to each session.

## Migration to v1.0.0 (versioning scheme change, filename stabilization)

Source as of this point was `2026.07.12-D` (all fixes below still apply
unchanged — this was a tooling/process migration, not a functional
change):
- Versioning switched from date-letter to plain semver. `1.0.0` is a
  clean reset, not a translation of the old scheme.
- Filenames stabilized: `SW26_GrblHAL_Mill_3axis_Metric.SRC/.LIB/.lng`,
  no embedded date.
- Fixed a real bug found during migration: `.SRC`'s `:LIBRARY=`
  self-reference had drifted from the actual `.LIB` filename (dot vs.
  underscore in the embedded date) — corrected.
- Release process automated via GitHub Actions
  (`.github/workflows/release-build.yml` + `.scripts/bump-release.sh` +
  `.scripts/attach-ctl.sh`), gated so nothing publishes until the `.ctl`
  is compiled and hardware-tested locally.
...
## Current version: `2026.07.12-D` (superseded by `1.0.0` above — see migration note)
Files: `SW26_GrblHAL_Mill_3axis_Metric_2026.07.12-D.SRC/.LIB/.lng` (now `SW26_GrblHAL_Mill_3axis_Metric.SRC/.LIB/.lng`)
